...
void andor(TablaHash** agenda, int* bands, char* nombre, char* apellido, int* edad_pointer, char* tel, int funcion) {
    ...
    if (funcion == 1) {
        ...
        imprimir_todos(*agenda, lista_nombre, lista_apellido, lista_edad, lista_tel);
    }
...
void imprimir_datos_correctos(TablaHash* agenda, int* bands, SList lista_comparar, char* nombre, char* apellido, int edad, char* tel) {
    ...
    while (iter != NULL) {
        ...
        if (bands[0]) {
            if (!strcmp(contacto->nombre, nombre)) suma++;
        } else suma++;
        if (bands[1]) {
            if (!strcmp(contacto->apellido, apellido)) suma++;
        } else suma++;
        if (bands[2]) {
            unsigned edad_unsigned = edad;
            if (contacto->edad == edad_unsigned) suma++;
        } else suma++;
        if (bands[3]) {
...
void imprimir_todos(TablaHash* agenda, SList lista_nombre, SList lista_apellido, SList lista_edad, SList lista_tel) {
